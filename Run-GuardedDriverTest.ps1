...
if ($devconExit -eq 0 -and $instanceId -and (-not $useRebootBind)) {
    Write-Log "Forcing no-reboot device restart after driver bind so the miniport service is actually loaded."
    pnputil /restart-device $instanceId *>> $logPath
    Write-Log "post-bind restart exit code: $LASTEXITCODE"
    Write-Log "Forcing devcon instance restart after driver bind."
    & $devconPath restart "@$instanceId" *>> $logPath
    Write-Log "post-bind devcon restart exit code: $LASTEXITCODE"
    Write-Log "Forcing no-reboot disable/enable cycle after driver bind."
    pnputil /disable-device $instanceId /force *>> $logPath
    Write-Log "post-bind disable exit code: $LASTEXITCODE"
    Start-Sleep -Seconds 2
    pnputil /enable-device $instanceId *>> $logPath
    Write-Log "post-bind enable exit code: $LASTEXITCODE"
    $boundService = $null
    try {
        $boundService = (Get-PnpDeviceProperty -InstanceId $instanceId -KeyName DEVPKEY_Device_Service -ErrorAction SilentlyContinue).Data
    } catch {
        $boundService = $null
    }
    if ($boundService) {
        Write-Log "Attempting explicit kernel service start for diagnostic load check: $boundService"
        sc.exe start $boundService *>> $logPath
        Write-Log "post-bind sc start exit code: $LASTEXITCODE"
        sc.exe query $boundService *>> $logPath
        Write-Log "Restarting device after explicit service load."
        & $devconPath restart "@$instanceId" *>> $logPath
        Write-Log "post-service-load devcon restart exit code: $LASTEXITCODE"
        pnputil /restart-device $instanceId *>> $logPath
        Write-Log "post-service-load pnputil restart exit code: $LASTEXITCODE"
    } else {
        Write-Log "No bound service found after enable; skipping explicit service start."
    }
