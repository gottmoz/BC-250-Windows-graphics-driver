...
$bak=Join-Path $root 'amdbc250_kmd.c.longrun.bak'
$log=Join-Path $root 'tools\headless_longrun.log'
...
L 'START longrun'
$variants=@(
  @{name='L1_h1_s1';h='1';s='1'}, @{name='L2_h1_s0';h='1';s='0'},
  @{name='L3_h0_s1';h='0';s='1'}, @{name='L4_h0_s0';h='0';s='0'}
)
for($round=1;$round -le 3;$round++){
  foreach($v in $variants){
    L "BEGIN round=$round $($v.name)"
    try {
      ...
      if($LASTEXITCODE -ne 0){ L "FAIL round=$round $($v.name) build=$LASTEXITCODE"; continue }
      ...
      if($LASTEXITCODE -ne 0){ L "FAIL round=$round $($v.name) inf2cat=$LASTEXITCODE"; continue }
      ...
      powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'Run-GuardedDriverTest.ps1') -TimeoutSeconds 30 -NoRebootRecovery -AllowBaselineBypass | Out-Null
      $ev=Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-Kernel-PnP/Configuration'; StartTime=(Get-Date).AddMinutes(-3)} -ErrorAction SilentlyContinue | Where-Object { $_.Id -eq 411 -and $_.Message -match 'VEN_1002&DEV_13FE' } | Select-Object -First 1
      if($ev){ $msg=$ev.Message -replace "`r`n",' | '; L "RESULT round=$round $($v.name) $msg" } else { L "RESULT round=$round $($v.name) no_411" }
    } catch { L "EXCEPTION round=$round $($v.name) $($_.Exception.Message)" }
  }
}
Copy-Item $bak $src -Force
L 'END longrun'
