...
function New-CallbackBlock([int]$count) {
  if($count -lt 1 -or $count -gt $callbackLines.Count){ throw "Invalid count $count" }
  (($callbackLines | Select-Object -First $count) -join "`r`n") + "`r`n"
}

function Build-And-Run([int]$count) {
  $name = "PFX$count"
...
"@
  ...
  $cbBlock = New-CallbackBlock -count $count
  $patCallbacks = 'InitialData\.DxgkDdiAddDevice[\s\S]*?InitialData\.DxgkDdiSystemDisplayWrite\s*=\s*BddDdiSystemDisplayWrite;\r?\n'
  $raw = [Regex]::Replace($raw, $patCallbacks, $cbBlock, [System.Text.RegularExpressions.RegexOptions]::Singleline)

  $driverEntryPat = 'DriverEntry\([\s\S]*?\{\r?\n\s*PAGED_CODE\(\);'
  $driverEntryRepl = '$0' + "`r`n`r`n    Q2SampleBreadcrumb(pRegistryPath, 1, 2, 0, 0, $count, 0);"
  $raw = [Regex]::Replace($raw, $driverEntryPat, $driverEntryRepl, [System.Text.RegularExpressions.RegexOptions]::Singleline)

  $replaceCall = @"
Q2SampleBreadcrumb(pRegistryPath, 2, 2, (ULONG)InitialData.Version, (ULONG)sizeof(InitialData), $count, 0);
    NTSTATUS Status = DxgkInitializeDisplayOnlyDriver(pDriverObject, pRegistryPath, &InitialData);
    Q2SampleBreadcrumb(pRegistryPath, 3, 2, (ULONG)InitialData.Version, (ULONG)sizeof(InitialData), $count, (ULONG)Status);
"@
  ...
  Write-Host ("RUN_PROFILE=$name|CB=$count|SYS=$sys")
...
Build-And-Run -count 21
Build-And-Run -count 17
Build-And-Run -count 19
Build-And-Run -count 20
